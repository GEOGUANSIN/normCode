...
def create_imperative_orchestrator_repos():
    ...
    prompt_name = "add_and_structure_prompt_with_thinking.txt"
    prompt_dir = "prompts"
    prompt_path = os.path.join(prompt_dir, prompt_name)
    # prompt_path = os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), prompt_dir, prompt_name))
    ...
    concept_entries = [
        ...
        ConceptEntry(id=str(uuid.uuid4()), concept_name='nominalized_prompt', type='{}', is_ground_concept=True),
    ...
    concept_repo.add_reference('nominalized_prompt', [f"%{{prompt_template}}({prompt_path})"], axis_names=['nominalized_prompt'])
    ...
    inf_values = [
        ...
        concept_repo.get_concept('nominalized_prompt')
    ]
...
if __name__ == "__main__":
    ...
    script_to_generate = os.path.join(examples_dir, "generated_scripts", "add_and_structure_for_orchest.py")
