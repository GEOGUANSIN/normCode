...
-   **Output:** A JSON object containing:
    ...
    2.  **`answer`** with:
       - **`initial_context_manifest`**: The complete manifest structure
       - **`file_mapping`**: Mapping from source files to target paths
    ```json
    {
...
- **`answer`**: A dictionary containing:
  - **`initial_context_manifest`**: The complete JSON manifest object (with `summary` and `sections` array)
  - **`file_mapping`**: A dictionary mapping source file paths (from input) to target paths in `context_store` (e.g., `{"prompt.md": "./context_store/raw--prompt.md"}`)
