...
Normcode_Analysis = """
<- {subject_analysis} | 1. imperative
    <= ::(analyze the subject {1}<$([{subject}, {verb}, {object}])%_> and provide a brief description)
...
def create_repositories(paragraph_text: str):
    ...
    concept_entries = [
        ...
        ConceptEntry(
            ...
            type="{}",
            axis_name="species",
            description="A brief analysis of the species of the first subject found.",
        ...
        ConceptEntry(
            id=str(uuid.uuid4()),
            concept_name="::(analyze the species of the {1}<$({subject})%_> and provide a brief description)",
            type="::({})",
            description="An imperative to analyze the species of the extracted subject.",
            is_ground_concept=True,
            is_invariant=True,
            reference_data=["::(analyze the species of the {1}<$({subject})%_> and provide a brief description)"],
            reference_axis_names=["_none_axis"],
        ),
    ]
    ...
    inference_entries = [
        ...
        InferenceEntry(
            id=str(uuid.uuid4()),
            inference_sequence='imperative',
            concept_to_infer=concept_repo.get_concept('{species}'),
            function_concept=concept_repo.get_concept('::(analyze the species of the {1}<$({subject})%_> and provide a brief description)'),
            value_concepts=[
