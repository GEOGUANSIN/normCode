...
:<:({a}) | 1. assignment
    ...
    <- {a}<$={2}> | 1.3. quantifying
        <= *every([all {a}]<$={1}>)@(1) | 1.3.1. assignment
            ...
            <- [all {a}]<$={1}> | 1.3.1.2. grouping
                <= &across({a})
                <- {a}<$={2}>

            <- <a>5> | 1.3.1.3. judgement
                <= ::{%(all True)}<{1}<$({number})%_> > 5>
                <- [all {a}]<$={1}>*1<:{1}>

            <- {a}<$={2}>  | 1.3.1.4. assignment
                <= $.([all {a}]<$={1}>*1) | 1.3.1.4.1. timing
                    <= @if(<a>5>)
                    <- <a>5>
                <- [all {a}]<$={1}>*1


            <- {a}<$={3}> | 1.3.1.5. imperative
                <= ::({1}<$({number})%> += 1)
                    <= @if!(<a>5>)
                    <- <a>5>
                <- [all {a}]<$={1}>*1<:{1}>
...
def create_repositories():
    ...
    concept_entries = [
        ...
        ConceptEntry(
            id=str(uuid.uuid4()),
            concept_name="::{%(all True)}<{1}<$({a})%_> is 5>",
            type="::",
    ...
    inference_entries = [
        ...
        InferenceEntry(
            ...
            function_concept=concept_repo.get_concept('$.({a}<$={2}>)'),
            value_concepts=[
                concept_repo.get_concept('{a}<$={2}>')
            ],
            flow_info={'flow_index': '1'},
            working_interpretation={"syntax": {"marker": ".", "assign_source": "{a}<$={2}>", "assign_destination": "{a}"}}
        ),
        ...
        InferenceEntry(
            ...
            working_interpretation={
                "syntax": {
                    ...
                    "timing_marker": "if", "condition_concept": "<a>5>"
                }
            }
        ),
