...
from _syntax._model_runner import ModelSequenceRunner
from _syntax.model_state import (
    ModelEnvSpecLite,
...
from _syntax._prompt import PromptTool
...
class Body:
	def __init__(self) -> None:
		...
		try:
			from _language_models import LanguageModel as _LM
...
def imperative_v2(self: Inference, input_data: Dict[str, Any] | None = None) -> States:
	...
	# IWI
	states = input_working_interpretation(
	...
	# MFP
	states = model_function_perception(states)
	log_states_progress(states, "MFP", "MFP")
	# MVP
	states = memory_value_perception(states)
	log_states_progress(states, "MVP", "MVP")
	# TVA
	states = tool_value_actuation(states)
	log_states_progress(states, "TVA", "TVA")
	# TIP
	states = tool_inference_perception(states)
	log_states_progress(states, "TIP", "TIP")
	# MIA
	states = memory_inference_actuation(states)
